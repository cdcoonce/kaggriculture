...
class TestResolveAgent:
    ...
    def test_agent_config_flows_goose_min_day_to_policy_config(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        # The JSON-shaped-config wiring proof for goose_min_day (kaggriculture,
        # 2026-09-11): a CLI --agent-config '{"goose_min_day": 6}' must reach
        # make_policy as an actual PolicyConfig with the field set, not get
        # silently dropped -- mirrors
        # test_agent_config_flows_ne_land_min_day_and_animal_buy_order_to_policy_config
        # above.
        import agent.policy as policy_module
        ...
        monkeypatch.setattr(policy_module, "make_policy", fake_make_policy)
        resolve_agent("champion", {"goose_min_day": 6})

        config = captured["policy_config"]
        assert isinstance(config, policy_module.PolicyConfig)
        assert config.goose_min_day == 6

    def test_agent_config_rejects_an_out_of_range_goose_min_day(self) -> None:
        with pytest.raises(ValueError, match="goose_min_day"):
            resolve_agent("champion", {"goose_min_day": -1})
        with pytest.raises(ValueError, match="goose_min_day"):
    ...
    def test_agent_config_flows_hire_slot_floor_to_policy_config(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        # The JSON-shaped-config wiring proof for hire_slot_floor
        # (kaggriculture, 2026-09-12): a CLI --agent-config
        # '{"hire_slot_floor": 4}' must reach make_policy as an actual
        # PolicyConfig with the field set, not get silently dropped --
        # mirrors test_agent_config_flows_zone_fallthrough_multiplier_to_
        # policy_config above.
        import agent.policy as policy_module
        ...
        monkeypatch.setattr(policy_module, "make_policy", fake_make_policy)
        resolve_agent("champion", {"hire_slot_floor": 4})

        config = captured["policy_config"]
        assert isinstance(config, policy_module.PolicyConfig)
        assert config.hire_slot_floor == 4

    def test_agent_config_rejects_an_out_of_range_hire_slot_floor(self) -> None:
        with pytest.raises(ValueError, match="hire_slot_floor"):
            resolve_agent("champion", {"hire_slot_floor": -1})
        with pytest.raises(ValueError, match="hire_slot_floor"):
            resolve_agent("champion", {"hire_slot_floor": 11})

    def test_agent_config_flows_strawberry_plant_cutoff_day_to_policy_config(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        # The JSON-shaped-config wiring proof for strawberry_plant_cutoff_day
        # (kaggriculture, 2026-09-12): a CLI --agent-config
        # '{"strawberry_plant_cutoff_day": 16}' must reach make_policy as an
        # actual PolicyConfig with the field set, not get silently dropped --
        # mirrors test_agent_config_flows_hire_slot_floor_to_policy_config
        # above.
        import agent.policy as policy_module
        ...
        monkeypatch.setattr(policy_module, "make_policy", fake_make_policy)
        resolve_agent("champion", {"strawberry_plant_cutoff_day": 16})

        config = captured["policy_config"]
        assert isinstance(config, policy_module.PolicyConfig)
        assert config.strawberry_plant_cutoff_day == 16

    def test_agent_config_rejects_an_out_of_range_strawberry_plant_cutoff_day(self) -> None:
        with pytest.raises(ValueError, match="strawberry_plant_cutoff_day"):
            resolve_agent("champion", {"strawberry_plant_cutoff_day": -1})
        with pytest.raises(ValueError, match="strawberry_plant_cutoff_day"):
            resolve_agent("champion", {"strawberry_plant_cutoff_day": 30})
